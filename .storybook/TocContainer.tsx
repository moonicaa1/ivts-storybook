...
/** 커스텀 docs 툴바 높이와 동일 — `.storybook/TocContainer` toolbar `height: 44` */
const DOCS_TOOLBAR_HEIGHT_PX = 44;
/** 우측 TOC를 뷰포트 상단(툴바 바로 아래)에 고정 */
const TOC_STICKY_TOP_PX = DOCS_TOOLBAR_HEIGHT_PX + 8;
...
export function TocContainer({ children, context }: DocsContainerProps) {
  ...
  const [pos, setPos] = useState<TocPos>({ left: 0, ready: false });
  ...
  useEffect(() => {
    ...
    // 3. 현재 문서에 이미 있는 iframe만 적용 (Observer·폴링·전역 getComputedStyle 스캔 제거 — 미리보기 멈춤 방지)
    document
      .querySelectorAll<HTMLIFrameElement>("iframe")
      .forEach((f) => applyDarkToIframe(f, dark));
  ...
  const calcPos = useCallback(() => {
    ...
    if (!content) {
      setPos({ left: 0, ready: false });
      return;
    }

    const contentRect = content.getBoundingClientRect();
    if (contentRect.right + 176 + 16 > window.innerWidth) {
      setPos({ left: 0, ready: false });
      return;
    }

    setPos({ left: contentRect.right + 16, ready: true });
  }, []);

  const tocScanTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const headingsSigRef = useRef<string>("");
  ...
  useEffect(() => {
    const runScan = () => {
      ...
      const sig = items.map((h) => `${h.id}:${h.text}`).join("|");
      if (sig !== headingsSigRef.current) {
        headingsSigRef.current = sig;
        setHeadings(items);
        setActiveId((prev) => {
          const still = items.some((h) => h.id === prev);
          return still ? prev : (items[0]?.id ?? "");
        });
      }
    ...
    const scheduleScan = () => {
      if (tocScanTimerRef.current) clearTimeout(tocScanTimerRef.current);
      tocScanTimerRef.current = setTimeout(() => {
        tocScanTimerRef.current = null;
        runScan();
      }, 800);
    };
  ...
  const toolbarBase: React.CSSProperties = {
    fontFamily:
      '"Pretendard Variable", Pretendard, "Segoe UI", -apple-system, sans-serif',
    fontSize: "13px",
    lineHeight: 1,
  };
    ...
    <>
      ...
      >
        ...
        <div
          style={{
            width: "1px",
            height: "20px",
            backgroundColor: tk.inputBorder,
            margin: "0 8px",
          }}
        />
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          <span
            style={{
              ...toolbarBase,
              fontWeight: dir === "ltr" ? 600 : 400,
              color: dir === "ltr" ? tk.textActive : tk.textMuted,
              cursor: "pointer",
              userSelect: "none",
            }}
            onClick={() => setDir("ltr")}
          ...
          <span
            style={{
              ...toolbarBase,
              fontWeight: dir === "rtl" ? 600 : 400,
              color: dir === "rtl" ? tk.textActive : tk.textMuted,
              cursor: "pointer",
              userSelect: "none",
            }}
            onClick={() => setDir("rtl")}
      ...
      {headings.length > 0 && pos.ready && (
        <aside
          ...
          style={{
            position: "fixed",
            top: TOC_STICKY_TOP_PX,
            left: pos.left,
            width: "176px",
            maxHeight: `calc(100vh - ${TOC_STICKY_TOP_PX}px - 16px)`,
            overflowY: "auto",
